...
def prompt_move(label: str, attacker: dict) -> tuple[dict, str, int, str, int]:
    ...
    atk_boost = clamp_boost(
        int(
            input(f"  Attacker {atk_stat.upper()} stage boost (-6 to +6, 0 if none): ")
            or 0
        )
    )
    ...
    def_boost = clamp_boost(
        int(
            input(f"  Defender {def_stat.upper()} stage boost (-6 to +6, 0 if none): ")
            or 0
        )
    )
...
def prompt_field(label: str) -> dict:
    ...
    terrain_in = (
        input("  Terrain (Electric/Grassy/Misty/Psychic, or blank): ").strip().lower()
    )
    while terrain_in not in TERRAINS:
        terrain_in = (
            input("  Please type Electric/Grassy/Misty/Psychic, or leave blank: ")
            .strip()
            .lower()
        )
    ...
    return {
        ...
        "isHelpingHand": input("  Helping Hand active for attacker? (y/n): ").lower()
        == "y",
        "isFriendGuard": input(
            "  Friend Guard active for defender's side? (y/n): "
        ).lower()
        == "y",
    }
...
def optimise_two(
    ...
    if same_stat:
        ...
        for delta_def in range(0, min(budget, 64) + 1):
            ...
            r1 = calc_damage(
                attacker1,
                {**defender_base, "boosts": {def_stat: def_boost1}},
                move1,
                field1,
            )
            r2 = calc_damage(
                attacker2,
                {**defender_base, "boosts": {def_stat: def_boost2}},
                move2,
                field2,
            )
    ...
    # ── three-stat search ─────────────────────────────────────────────────────
        ...
        for delta_a in range(0, cap + 1):
            for delta_b in range(0, cap - delta_a + 1):
                ...
                r1 = calc_damage(
                    attacker1,
                    {**defender_base, "boosts": {stat_a: def_boost1}},
                    move1,
                    field1,
                )
                r2 = calc_damage(
                    attacker2,
                    {**defender_base, "boosts": {stat_b: def_boost2}},
                    move2,
                    field2,
                )
                ...
                row = {
                    ...
                    f"{stat_b}_SP": B_SP,
                    "delta_hp": delta_hp,
                    f"delta_{stat_a}": delta_a,
                    f"delta_{stat_b}": delta_b,
                    "dmg1": dmg1,
                    "dmg2": dmg2,
                    "HP": HP,
                    "pct1": pct1,
                ...
                print(
                    f"+{delta_hp:>2} HP / +{delta_a:>2} {stat_a.upper()} / +{delta_b:>2} {stat_b.upper()}  "
...
def _print_result_two_stat(opt: dict, minimum_sum: float, def_stat: str):
    ...
    print("─" * 60)
    print(
        f"  Spread:     {opt['HP_SP']} HP / {opt[f'{def_stat}_SP']} {def_stat.upper()}"
    )
    print(
        f"  Additional: +{opt['delta_hp']} HP / +{opt[f'delta_{def_stat}']} {def_stat.upper()}"
    )
    ...
    print(f"              {opt['desc2']}")
    print(
        f"  Combined:   {opt['dmg1'] + opt['dmg2']} / {opt['HP']} HP  "
...
def _print_result_three_stat(opt: dict, minimum_sum: float, stat_a: str, stat_b: str):
    ...
    print("─" * 60)
    print(
        f"  Spread:     {opt['HP_SP']} HP / {opt[f'{stat_a}_SP']} {stat_a.upper()} / {opt[f'{stat_b}_SP']} {stat_b.upper()}"
    )
    print(
        f"  Additional: +{opt['delta_hp']} HP / +{opt[f'delta_{stat_a}']} {stat_a.upper()} / +{opt[f'delta_{stat_b}']} {stat_b.upper()}"
    )
    print(f"  Move 1:     {opt['dmg1']} / {opt['HP']} HP  ({opt['pct1']:.1f}%)")
    print(f"              {opt['desc1']}")
    print(f"  Move 2:     {opt['dmg2']} / {opt['HP']} HP  ({opt['pct2']:.1f}%)")
...
if def_stat1 != def_stat2:
    existing_def2 = int(
        input(f"  SPs already in {def_stat2.upper()} (0 if none): ") or 0
    )
else:
    existing_def2 = existing_def1

stat_label = (
    f"HP + {def_stat1.upper()}"
    if def_stat1 == def_stat2
