...
**The 66-total-SP cap** — Champions caps SP two ways: 32 per stat, 66 total across every stat on the Pokémon. The per-stat cap is checked everywhere (`HP_SP > 32` etc.), but only the 3D branch above needs an explicit *total* check (`HP_SP + DEF_SP + SPD_SP > 66`) — it's the only place that ever tracks 3 stats at once, and `3 × 32 = 96 > 66`. Every other sweep in the codebase (`single/calc.py`, `single/survive.py`, `multi/calc.py`'s same-stat branch) only ever tracks 2 stats, and `2 × 32 = 64 ≤ 66`, so the per-stat checks alone already guarantee the total stays under 66 — no separate check needed there (each has a comment noting this). If you ever add a third tracked stat to one of those 2-stat branches, this stops being true and needs the same explicit total check. `parse_config()`/`parse_config_multi()` also assert `EXISTING_*_SP <= 32` upfront (and, for multi, their sum `<= 66`) so a bad config value fails with a clear message instead of just silently filtering out every point in the sweep.

This also affects the `cap` each branch iterates up to. Every branch spends its whole delta budget as an exact partition (`delta_hp + delta_def + ... == cap`, always, by construction), so `cap` needs to equal the true reachable ceiling for that sum, not an arbitrary bound — undershooting it silently prunes valid, potentially-better spreads from the search rather than raising an error. For 2-stat branches that ceiling is `min(budget, 64)` (2 × 32). For the 3-stat branch it's `min(budget, 66)`, **not** 64 — the binding constraint there is the 66-total cap, not `3 × 32 = 96`. (This was itself a live bug during development: the 3-stat branch was copy-pasted from the 2-stat one with `cap = min(budget, 64)` intact, silently capping exploration below the true 66 ceiling — caught by inspection, not by a test, so if you touch either `cap` line, double check which ceiling actually binds for that branch's stat count before assuming 64 is universal.)
